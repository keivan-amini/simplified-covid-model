...
#--------------------------------------------------------------------------------------
# MOBILITY VS SOCIAL ACTIVITY: From another work (dr. Lunedei) we estrapolated some Social activity rate coefficient.
# Social activity rate
m_vals = np.array([1.00, 1.00, 1.00, 0.24, 0.16, 0.17, 0.18, 0.24, 0.24, 0.24, 0.34, 0.24, 0.21, 0.27, 0.25, 0.23, 0.29, 0.26, 0.20, 0.38, 0.28, 0.70, 0.55, 0.85, 0.54, 0.38], dtype = np.double)
# Confinement measures dates
m_days = np.array([   0,   54,   61,   71,   83,  125,  139,  155,  167,  181,  258,  299,  320,  341,  359,  375,  398,  418,  429,  468,  482,  537,  560,  590,  612,  635], dtype = np.int)
#                        23feb 01mar 11mar 23mar 04mag 18mag 03giu 15giu 29giu 14set 25ott 15nov 06dic 24dic 09gen 01feb 21feb 04mar 12apr 26apr 20giu 13lug 12ago  3set 26set
...
df_2021 = pd.read_csv(url_2021,sep = ';') #upload the mobility 2021 dataset

ordered2021_df = df_2021.sort_values(by = 'data') 
ordered2021_df['Average Daily Mobility'] = ordered2021_df[col_list].sum(axis=1) / 24

global2021_df = pd.pivot_table(
    ordered2021_df,
    index='data',
    columns='Nome via',
    values='Average Daily Mobility',
...
total_mobility_2021 = global2021_df['All']
lastElementIndex = len(total_mobility_2021)-1
total_mobility_2021 = total_mobility_2021[:lastElementIndex]

smoothed_total_2021 = total_mobility_2021.rolling(7, center=True).mean()
days2021 = list(range(367,731))

plt.scatter(days,smoothed_total/smoothed_total.max(), label = "Mobility 2020", color = "green", marker = "o", s = 4)
plt.scatter(days2021,smoothed_total_2021/smoothed_total_2021.max(), label = "Mobility 2021", color = "darkorange", marker = "o", s = 4)
plt.scatter(m_days,m_vals, label = 'Social Activity', color ='deeppink', marker = "+", s = 50) # normalized to 1
plt.axvline(x=70, color='b', ls='--', label='1st Lockdown') # start of the first italian lockdown, 9th march
plt.axvline(x=125, color='b', ls='--') # end lockdown, 18th may
plt.axvline(x=286, color='c', ls='--', label='Start of the 2nd wave') # start of the second wave, 13th october, mandatory mask
plt.axvline(x=297, color='y', ls='--', label='Closure of activities') # 24th october: chiusura attività
...
plt.ylabel('Average number of motor vehicle detected normalized')
plt.legend(loc="lower right")
plt.title('Total Mobility vs Time in Bologna during 2020 and 2021')
plt.show()

# Let's operate some shift.
mobility_2020 = smoothed_total/smoothed_total.max()
mobility_2021 = smoothed_total_2021/smoothed_total_2021.max()

shift_1 = mobility_2020[165] - m_vals[7] #165 -> 14 giugno

for index in range(7,len(m_vals)): # dall'ottavo elemento in poi shifto tutti i valori di 0,57
        m_vals[index] += shift_1


plt.scatter(days,smoothed_total/smoothed_total.max(), label = "Mobility", color = "green", marker = "o", s = 4)
plt.scatter(days2021,smoothed_total_2021/smoothed_total_2021.max(), color = "green", marker = "o", s = 4)
plt.scatter(m_days,m_vals, label = 'Social Activity', color ='deeppink', marker = "+", s = 60) # normalized to 1
plt.axvline(x=366, color='darkturquoise', ls='dotted', label = 'End of the year')
plt.axvspan(155, 731, facecolor='lightsalmon', alpha=0.3,label = 'Shift = +' + str(round(shift_1,2)))
plt.legend(loc="lower right")
plt.title('Mobility & Sociality vs Time (sociality shifted + 0.57)')
plt.show()


#secondo shift: primi 4 elementi del 2021
shift_2 = mobility_2021[10] - m_vals[15]
for index in range(15,19):
        m_vals[index] += shift_2


#terzo shift: 20 giugno, 537esimo giorno
shift_3 = mobility_2021[172] - m_vals[21]
for index in range(21,len(m_vals)):
        m_vals[index] += shift_3
